...
filter_association_first_token <- function(index){
...
NLP_filter_second_member_section_title <- function(occurrences){
  ...
  for (index in occurrences){
    if (filter_association_first_token(index)){return(index)}
  }}
...
head(unique(material_and_method_section$sentence), 30)
