...
eliminate_cf_occurrences <- function(x, occurrences) {
  ...
  for (occur in occurrences) { 
    ...
    head_lemma<-grep_lemma_head_token_id(x, occur)
    if (head_lemma=="discuss") {
      next
    }
...
grep_lemma_head_token_id <- function(x, index){
  ...
  lemma_head_token_id<-x[which(x$sentence_id==sentence_id)[head_token_id],]$lemma
